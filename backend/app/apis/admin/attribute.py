...
@router.post("/{attribute_id}/options", status_code=status.HTTP_201_CREATED)
async def add_attribute_option(
    db: db_dependency,
    admin: admin_dependency,
    attribute_id: int = Path(gt=0),
    option_data: AttributeOptionCreate = None,
):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to add attribute option.",
        )


@router.put("/{attribute_id}/options/{option_id}", status_code=status.HTTP_200_OK)
async def update_attribute_option(
    ...
    PUT /admin/attributes/{id}/options/{option_id}
    """
...
@router.delete("/{attribute_id}/options/{option_id}", status_code=status.HTTP_200_OK)
async def delete_attribute_option(
    ...
    DELETE /admin/attributes/{id}/options/{option_id}
    """
